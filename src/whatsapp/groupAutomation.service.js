...
const getMemberNumber = (sock, jid) => {
...
const enabledRules = (manager, type) =>
...
const deleteMessage = async (sock, groupJid, msgKey) => {
  try {
    if (!msgKey) return;
...
const isScheduleDue = (schedule, now) => {
...
const runDueSchedules = async () => {
  try {
    ...
    for (const manager of managers) {
      ...
      const { getSocket } = await import("./whatsappManager.js");
